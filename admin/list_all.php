...
@$key=trim($_POST["key"]);
//是否存在关键字
$result=$lnk -> query("DESCRIBE attr_list_$menuid diy_key");
$field = mysqli_fetch_array($result);
$result2=$lnk -> query("DESCRIBE attr_list_$menuid diy_xinhao");
$field2 = mysqli_fetch_array($result2);
if($field[0] & $field2[0] ){
$where.= $key ? " where name like '%$key%' or diy_key like '%$key%' or diy_xinhao like '%$key%'":"";
}elseif($field[0]){
$where.= $key ? " where name like '%$key%' or diy_key like '%$key%'":"";
}else{
...
echo "</td>";
$xinhao=$rs["diy_xinhao"]?$rs["diy_xinhao"]:"";
echo "<td>".$rs["name"]." $xinhao </td>";  //标题
echo "<td>".$rs["px"]."</td>";  //排序
$citylist=($menuid==8)?"&nbsp;&nbsp;&nbsp;<a href='?act=addnew&menuid=$menuid'><span class='glyphicon glyphicon-plus'></span></a>":"";
